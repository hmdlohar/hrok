...
ws.on('message', (data) => {
    ...
    if (msg.type === 'TUNNEL_ASSIGNED') {
        myPublicPort = msg.publicPort;
        console.log(`Tunnel established! Your public port is: ${myPublicPort}`);
        console.log(`Traffic to port ${myPublicPort} will be forwarded to local port ${LOCAL_TARGET_PORT}`);
    } else if (msg.type === 'NEW_CONNECTION') {
        const { sessionId } = msg;
        console.log(`New connection request: ${sessionId}`);

        const localSocket = net.connect(LOCAL_TARGET_PORT, () => {
            console.log(`Connected to local service on port ${LOCAL_TARGET_PORT}`);
        });
        ...
        localSocket.on('error', (err) => {
            console.error(`Local socket error for session ${sessionId}:`, err);
            activeConnections.delete(sessionId);
        });
    } else if (msg.type === 'DATA') {
        const { sessionId, payload } = msg;
